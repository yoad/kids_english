...
function setupHome() {
  ...
  document.getElementById('home-name').textContent = isAlon?'אלון':'רומי';
  const p=getProgress();
...
function setupGames() {
  ...
  const games=[
    ...
    {id:'spelling',icon:'✏️',name:'אית את המילה',desc:state.profile==='romi'?'מצא את האות הראשונה':'סדר את האותיות',unlock:10},
...
function initSpelling() {
  ...
  // For Romi, use short words only
  let pool = state.profile==='romi' ? learned.filter(w=>w.word.length<=5) : learned;
  if(pool.length<3) pool=learned;
...
function showSpellingRound() {
  ...
  if(state.profile==='romi') setupRomiSpelling(w);
  else setupAlonSpelling(w);
}
function setupRomiSpelling(w) {
...
function setupAlonSpelling(w) {
  document.querySelector('.btn-undo').style.display='';
  const word=w.word.toUpperCase().replace(/\s/g,'');
  const slots=document.getElementById('spelling-slots'); slots.innerHTML='';
  for(let i=0;i<word.length;i++){
    const s=document.createElement('div'); s.className='spell-slot'; s.dataset.index=i; slots.appendChild(s);
  }
  const letters=shuffle(word.split(''));
  // Add 2-3 random distractor letters
  const alpha='ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  for(let i=0;i<2;i++) letters.push(alpha[Math.floor(Math.random()*26)]);
  const choices=document.getElementById('letter-choices'); choices.innerHTML='';
  shuffle(letters).forEach((l,i)=>{
    const btn=document.createElement('button');
    btn.className='letter-btn'; btn.textContent=l; btn.dataset.li=i;
    btn.onclick=()=>pickLetter(btn,l);
    choices.appendChild(btn);
  });
}
function pickLetter(btn,l) {
  const w=spellingGame.current.word.toUpperCase().replace(/\s/g,'');
  if(spellingGame.filled.length>=w.length) return;
  spellingGame.filled.push({letter:l,btnIdx:btn.dataset.li});
  btn.classList.add('used');
  const slotIdx=spellingGame.filled.length-1;
  const slot=document.querySelectorAll('.spell-slot')[slotIdx];
  slot.textContent=l; slot.classList.add('filled');
  // Check if complete
  if(spellingGame.filled.length===w.length){
    const attempt=spellingGame.filled.map(f=>f.letter).join('');
    if(attempt===w){
      document.getElementById('spelling-feedback').textContent='מעולה! 🎉';
      document.getElementById('spelling-feedback').className='game-feedback correct';
      spellingGame.score+=3;
      if(window.confetti) confetti({particleCount:80,spread:60,origin:{y:0.7}});
      setTimeout(()=>{spellingGame.round++;showSpellingRound();},1500);
    } else {
      document.getElementById('spelling-feedback').textContent='לא בדיוק, נסה שוב! 😊';
      document.getElementById('spelling-feedback').className='game-feedback wrong';
      document.querySelectorAll('.spell-slot').forEach(s=>{s.classList.add('wrong-shake');});
      setTimeout(()=>{
        // Reset
        spellingGame.filled=[];
        document.querySelectorAll('.spell-slot').forEach(s=>{s.textContent='';s.classList.remove('filled','wrong-shake');});
...
function undoLetter() {
  if(spellingGame.filled.length===0) return;
  const last=spellingGame.filled.pop();
  const slotIdx=spellingGame.filled.length;
  const slot=document.querySelectorAll('.spell-slot')[slotIdx];
  slot.textContent=''; slot.classList.remove('filled');
  const btn=document.querySelector(`.letter-btn[data-li="${last.btnIdx}"]`);
...
// ===== MEMORY GAME =====
function initMemory() {
  const pairs = state.profile==='romi' ? 4 : 6;
